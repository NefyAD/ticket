...
class CloseTicketView(discord.ui.View):
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message("チケットを閉じますか？", view=ConfirmCloseView(), ephemeral=True)
...
class ConfirmCloseView(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if dm_message or ticket_link:
            ...
            if ticket_link:
                embed.add_field(name="チケットリンク", value=f"[こちらをクリック]({ticket_link})", inline=False)
...
async def ticket_panel_command(interaction: discord.Interaction):
    ...
    embed = create_ticket_embed(
        title=title, description=description, image_url=guild_info.get("image_url"), thumbnail_url=guild_info.get("thumbnail_url"),
        color=guild_info.get("color", discord.Color.blue()), developed_text=guild_info.get("text"), developed_icon_url=guild_info.get("icon_url")
